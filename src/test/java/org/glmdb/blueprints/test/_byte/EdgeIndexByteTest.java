 ...
 * Time: 11:53 AM
 */
public class EdgeIndexByteTest extends BaseGlmdbGraphTest {
    ...
    public void testIndexRemoval() {
        ...
        try {
            ...
            Assert.assertEquals(2, thunderGraph.getDbEntries(DbEnum.EDGE_BYTE_INDEX));
            e1.remove();
            Assert.assertEquals(1, thunderGraph.getDbEntries(DbEnum.EDGE_BYTE_INDEX));
            e2.remove();
            Assert.assertEquals(0, thunderGraph.getDbEntries(DbEnum.EDGE_BYTE_INDEX));
        } finally {
    ...
    public void dropByteIndexTest() {
        ...
        try {
            ...
            Assert.assertEquals(3, count(thunderGraph.getEdges("name1", (byte)1).iterator()));
            Assert.assertEquals(3, thunderGraph.getDbEntries(DbEnum.EDGE_BYTE_INDEX));
    ...
    public void testUpdateValuesInIndexKeys() throws Exception {
        ...
        try {
            ...
            e1.setProperty("name", (byte)1);
            Assert.assertEquals(e1.getProperty("name"), (byte)1);
            Assert.assertEquals(1, count(graph.getEdges()));
            graph.commit();

            e1 = graph.getEdges("name", (byte)1).iterator().next();
            Assert.assertEquals(e1.getProperty("name"), (byte)1);
            e1.setProperty("name", (byte)2);
            Assert.assertEquals(e1.getProperty("name"), (byte)2);
            Assert.assertEquals(1, count(graph.getEdges()));
            graph.commit();


            Assert.assertFalse(graph.getEdges("name", (byte)1).iterator().hasNext());
            e1 = graph.getEdges("name", (byte)2).iterator().next();
            Assert.assertEquals(e1.getProperty("name"), (byte)2);
            Assert.assertEquals(1, count(graph.getEdges()));
            graph.commit();
    ...
    public void testPartialMatchFails() {
        ...
        try {
            ...
            Assert.assertEquals(1, count(graph.getEdges("name", (byte)1)));
            Assert.assertEquals(0, count(graph.getEdges("name", (byte)2)));
        } finally {
            graph.shutdown();
        }
    }
    ...
    public void testCursorRefreshOnFirst() {
        ...
        try {
            ...
            Assert.assertEquals(10, count(graph.getEdges("name", (byte)1)));
            Iterator<Edge> iter = graph.getEdges("name", (byte)1).iterator();
            ...
            Edge e = iter.next();
            Assert.assertEquals((byte)1, e.getProperty("name"));
        } finally {
            graph.shutdown();
        }
    ...
    public void testRemoveTransactionAlreadyWritableFail() {
        ...
        try {
            ...
            e.setProperty("name", (byte)2);
            Assert.assertEquals(0L, e.getId());
            Assert.assertEquals((byte)2, e.getProperty("name"));
            iter.remove();
            Assert.assertNull(graph.getEdge(0L));
            Assert.assertEquals(9, count(iter));

            graph.commit();
            iter = graph.getEdges("name", (byte)1).iterator();
            Assert.assertEquals(9, count(iter));
